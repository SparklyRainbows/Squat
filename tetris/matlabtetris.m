    ...
    function connectToServer(~, ~, ~)
        if ~connected
            try
                ...
                configureCallback(tcp, "byte", 2, @TCPCallbackFcn);
                connected = true;
                stop(S.tmr2)
            catch
        ...
        'StartDelay',1,... %
        'ExecutionMode','fixedrate',...
        'TimerFcn',@connectToServer); % Function def. below.
        start(S.tmr2)
        ...
        function TCPCallbackFcn(~, ~, ~)
            if connected && isfield(S, 'CUR')
            % if connected && isfield(S, 'CUR') && ~TCPCommandExecuted
                data = read(tcp);
                if ~isempty(data)
                    inputs = dataToStruct(data);
                    fig_kpfcn([], inputs);
                   % TCPCommandExecuted = 0;
                end
            end
        end
